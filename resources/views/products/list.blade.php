        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                ...
                @foreach($products as $product)
                    <a href="{{route('details', $product->id)}}" class="flex flex-row shadow-sm sm:rounded-lg border">
                        <div class="w-1/4 h-full shadow-sm sm:rounded-lg">
                            <img src="{{ $product->image ? '/build/assets/'.$product->image : '/build/assets/product.png' }}" />
                        </div>
